...
__inline__ void scale_bbox(cv::Mat& img, float bbox[4], int kInputW_, int kInputH_){
    float r_w = kInputW_ / (img.cols * 1.0);
    float r_h = kInputH_ / (img.rows * 1.0);
    float r = std::min(r_w, r_h);
    float pad_h = (kInputH_ - r * img.rows) / 2;
    float pad_w = (kInputW_ - r * img.cols) / 2;
