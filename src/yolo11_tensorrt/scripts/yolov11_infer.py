...
weight = "weights/yolov11n_v2_no_dla.engine"
...
class YoloDetector:
    def __init__(self):
        ...
        self.detect_time = 1.0
        # init and load
        self.model = YOLO(os.path.join(path_curr, weight), task='detect')
    ...
    def detect_callback(self, event):
        ...
        self.detect_time = (endTime-startTime).to_sec()
    ...
    def inference(self, ori_img):
        ...
        res_img = cv2.resize(ori_img, (640, 640), interpolation = cv2.INTER_LINEAR) 
    ...
    def postprocess(self, ori_img, output):
        ...
        freq = 1 / self.detect_time
        text = f"Freq: {freq:.2f} Hz"  # 保留两位小数
        cv2.putText(ori_img, text, (10, 30), 0, 0.7, (0, 0, 255), 2)
